...
- `src/index.ts` writes `dist/index.html`; `src/label.ts` emits each label as an HTML string; `src/css.ts` embeds print CSS.
...
Keep this a small static TypeScript generator—no server or interactive app is needed. V2 should add:

- explicit `build`, `typecheck`, preview, and asset-audit commands;
- configuration validation for IDs, ranges, crop values, logo references, and asset presence;
- a domain layer that turns catalog records into a label view model;
- separate render and styling modules rather than one long inline string;
- SVG inlining and a custom-size output suitable for vinyl-print proofing/production;
- structural/snapshot checks for representative layout cases.

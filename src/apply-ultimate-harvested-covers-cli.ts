...
const COVERS_PATH = path.join(process.cwd(), "docs", "MARVEL-ULTIMATE-COVER-URLS.json");
...
function formatLabels(labels: LabelConfig[], lineEnding: string): string { return `${JSON.stringify(labels, null, 4).replace(/\[\n\s+(-?\d+(?:\.\d+)?),\n\s+(-?\d+(?:\.\d+)?)\n\s+\]/g, "[$1, $2]")}${lineEnding}`; }
function valid(value: string | undefined): value is string { try { const url = new URL(value ?? ""); return url.hostname === "cdn.marvel.com" && /\/clean\.jpg$/i.test(url.pathname); } catch { return false; } }
function main(): void {

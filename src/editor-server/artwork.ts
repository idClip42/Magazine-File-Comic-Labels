...
import { imageCache, loadArtworkFromDiskCache, saveArtworkToDiskCache } from "./artwork-cache";
...
export const configuredArtworkAssets = new Set(labels.flatMap(label => [
    label.art.asset,
    ...(label.art.options ?? []),
]));
...
export async function preloadRemoteImages(): Promise<void> {
    ...
    const otherAssets = assetsToDownload.filter(asset => !isMarvelCdnImage(asset));
    ...
    const downloadAsset = async (asset: string): Promise<string | undefined> => {
    ...
    const worker = async () => {
        while (nextIndex < otherAssets.length) await downloadAsset(otherAssets[nextIndex++]);
    };
    const nonMarvelPreload = Promise.all(Array.from({ length: Math.min(preloadConcurrency, otherAssets.length) }, worker));
    ...
    for (let index = 0; index < marvelAssets.length; index += 1) {
        ...
        consecutiveMarvelBlocks = failure?.startsWith("403 ") || failure?.startsWith("429 ")
            ? consecutiveMarvelBlocks + 1
            : 0;
        if (consecutiveMarvelBlocks >= marvelBlockThreshold) {
            const remaining = marvelAssets.length - index - 1;
            console.warn(chalk.yellow(`Stopping Marvel CDN preload after ${marvelBlockThreshold} consecutive blocks; ${remaining} uncached Marvel image(s) will retry on the next start.`));
            break;
        }
    }
...
export function serveArtwork(request: http.IncomingMessage, response: http.ServerResponse): boolean {

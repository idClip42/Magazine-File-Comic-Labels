...
const coversPath = path.join(process.cwd(), "docs", "NON-MARVEL-COVER-URLS.json");
...
function main(): void {
  ...
  console.log(`${write ? "Applying" : "Would apply"} verified non-Marvel cover options to ${changed} label(s).`); if (write) fs.writeFileSync(labelsPath, `${JSON.stringify(labels, null, 4).replace(/\[\n\s+(-?\d+(?:\.\d+)?),\n\s+(-?\d+(?:\.\d+)?)\n\s+\]/g, "[$1, $2]")}${original.includes("\r\n") ? "\r\n" : "\n"}`);
}
main();

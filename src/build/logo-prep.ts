...
import { CategoriesConfig, LayoutConfig } from "../core/types";
...
function extractFill(pathTag: string): string | undefined {
    ...
    const styleFill = styleMatch?.[2].match(
        /(?:^|;)\s*fill\s*:\s*([^;]+)/i,
    )?.[1];
...
function addRole(pathTag: string, role: "primary" | "secondary"): string {
    const withoutExistingRole = pathTag.replace(
        /\sdata-logo-fill\s*=\s*(["']).*?\1/i,
        "",
    );
    return withoutExistingRole.replace(
        /<path\b/i,
        `<path data-logo-fill="${role}"`,
    );
}
...
function normalizeSvg(
    ...
    if (sourceFills.length === 0)
        throw new Error(`${source} has no supported filled paths.`);
    if (sourceFills.length > 2)
        throw new Error(
            `${source} has ${sourceFills.length} fill colors; V2 logos must use one black fill or black plus gray.`,
        );

    const sorted = [...sourceFills].sort(
...
export function prepareLogos(
    ...
    for (const [categoryId, category] of Object.entries(categories)) {
        for (const [logoId, logo] of Object.entries(category.logos)) {
            if (
                /^https?:\/\//i.test(logo.asset) ||
                !logo.asset.toLowerCase().endsWith(".svg")
            )
                continue;
    ...
    fs.writeFileSync(
        path.join(outputDirectory, "logo-preparation.json"),
